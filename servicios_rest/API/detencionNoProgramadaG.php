...
if(isset($_GET["fin"])&&($_GET["nom"])&&($_GET["ape"])&&($_GET['vel'])&&($_GET["det"])&&($_GET["fk"]&&($_GET["ord"]))){
    $fin=$_GET['fin'];
    $det=$_GET['det'];
    $nom=$_GET['nom'];
    $ape=$_GET['ape'];
    ...
    $consulta="INSERT INTO turno(inicio, nom_opera, ape_opera, velocidad_pro, fk_dnprog, fk_maquina,fk_tab_ext ) VALUES ('{$fin}', '{$nom}', '{$ape}', '{$vel}', '{$det}', '{$fk}', '{$ord}')";
