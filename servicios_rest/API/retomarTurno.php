...
if(isset($_GET["hora"])&&($_GET["fecha"])&&($_GET["nom"])&&($_GET["ape"])&&($_GET['vel'])&&($_GET["fk"]&&($_GET["ord"]))){
    $hora=$_GET['hora'];
    $fecha=$_GET['fecha'];
    $nom=$_GET['nom'];
    $ape=$_GET['ape'];
    ...
    $consulta="INSERT INTO turno(hora_inicio, fecha_inicio, nom_opera, ape_opera, velocidad_pro, fk_maquina,fk_tab_ext ) VALUES ('{$hora}', '{$fecha}', '{$nom}', '{$ape}', '{$vel}', '{$fk}', '{$ord}')";
