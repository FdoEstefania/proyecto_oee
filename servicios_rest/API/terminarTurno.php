...
if(isset($_GET["fin"])&&($_GET["cant"])&&($_GET["nom"])&&($_GET["ape"])&&($_GET['vel'])&&($_GET["fk"]&&($_GET["ord"]))){
    $fin=$_GET['fin'];
    $cant=$_GET['cant'];
    $nom=$_GET['nom'];
    $ape=$_GET['ape'];
    ...
    $consulta="INSERT INTO turno(fin, cant_prod, nom_opera, ape_opera, velocidad_pro, fk_maquina,fk_tab_ext ) VALUES ('{$fin}','{$cant}', '{$nom}', '{$ape}', '{$vel}', '{$fk}', '{$ord}')";
